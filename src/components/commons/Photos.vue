  ...
  <v-card min-height="400px">
    <v-row>
      <v-col cols="12">
        <div
          class="pa-4"
          style="background:#2c7873;display:flex;justify-content:space-between;flex-wrap:wrap;"
        >
          <h2 class="white--text">Photos</h2>
          <v-btn color="orange" dark rounded id="file-btn">
            <input type="file" hidden id="file-input" multiple @change="getSelectedFiles" />
      ...
      <v-col v-if="photos.length <= 0" cols="12" class="px-4">
        ...
        <v-row v-else justify="space-around">
          <v-sheet width="200" height="200" color="#f4f4f4" title="no_data">
            ...
            />
            <h3 style="font-weight:100;" class="mx-auto">No Photos uploaded yet !</h3>
          </v-sheet>
        </v-row>
      </v-col>
          ...
          >
           <v-row align="baseline">
              <v-col cols="12" class="text-center pa-0">
                 <v-btn class="pa-0 mr-3" small fab title="view" light><v-icon color="teal" @click="viewPhoto(i)">mdi-eye-outline</v-icon></v-btn> 
                 <v-btn small fab title="delete" light><v-icon color="red" @click="deletePhoto(i)">mdi-close</v-icon></v-btn> 
                 </v-col>
                  <v-col v-if="itemToDelete == i" cols="12" class="py-0">
                    <div class="white text-center" style="height:auto">
                      <h5 class="ma-0 pa-0" style="font-size:0.8rem;color:#ff0044;height: 10px;line-height: 2.5">Are you sure ?</h5>
                      <v-btn small text color="success" @click="deletePhotoParmanent">yes</v-btn>
                      <v-btn small text color="red" @click="cancelDelete">NO</v-btn>
                    </div>
                  </v-col>
    ...
    <v-dialog v-model="dialog" fullscreen hide-overlay transition="dialog-bottom-transition">
      <v-card>
        ...
        <v-list three-line subheader>
          <v-subheader>
            <v-btn color="orange" @click="uploadPhotos" :loading="upload_all" dark>
              <v-icon class="mr-1">mdi-arrow-up-bold-circle-outline</v-icon>Upload all
            </v-btn>
          </v-subheader>
          <v-list-item>
            <v-list-item-content>
              <v-row>
                <v-col cols="12">
                  <v-row>
                    <v-col cols="4" class="py-1" v-for="(item,i) in files" :key="i" :id="'card-'+i">
                      <v-card >
                        <v-card-text class="py-0">
                          <v-row align="center">
                            ...
                            <v-col cols="4">
                              <v-btn outlined fab small color="red" class="mr-2" @click="removeItemFromUpload(i)">
                                <v-icon>mdi-close</v-icon>
                              </v-btn>
...
<v-dialog v-model="showPhoho" fullscreen hide-overlay transition="dialog-bottom-transition">
      <v-card>
        <v-toolbar dark color="teal">
         ...
         <v-card-text>
          <galleryView :img_url="img_url" :pics="photos" :onboarding="onboarding" />
        </v-card-text>
      </v-card>
</v-dialog>
...
export default {
  ...
  methods: {
    ...
    deletePhotoParmanent(){
      ...
      axios.delete(urls.URL+"/gallery/delete/"+this.photos[this.itemToDelete].id,config)
      .then(res=>{
        if(res.data.status){
